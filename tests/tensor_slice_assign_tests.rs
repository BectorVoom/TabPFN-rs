...
mod cpu_tests {
    ...
    fn test_precision_preservation() {
        ...
        let slice = result.slice(ranges.clone());
...
mod performance_tests {
    ...
    fn test_multiple_sequential_updates() {
        ...
        for (ranges, value) in updates.iter() {
            ...
            tensor = update_tensor_slice_4d(tensor, values, *ranges).unwrap();
        }
        
        // Verify each quadrant has the correct value
        for (ranges, expected_value) in updates.iter() {
            assert!(validate_slice_values(&tensor, *ranges, *expected_value, 1e-6));
        }
    }
}