...
use tab_pfn_rs::tabpfn::architectures::base::{
    ...
    transformer::{
        PerFeatureTransformer, TorchRngContext, DataDAG, NodeMetadata
    },
};
...
/// 
/// This test verifies that TorchRngContext::with_isolated_seed actually
...
fn test_a1_rng_isolation() {
    ...
    // Test 1: Verify isolation doesn't affect global state
    // Since our implementation uses StdRng CPU generation, we test the 
    // isolation flag behavior and deterministic output
    let rng_context = TorchRngContext::<TestBackend>::new(123, device);
    
    // Verify initial state
    assert!(!rng_context.is_isolated(), "Initial state should not be isolated");
...
fn test_a2_learned_embedding_functional() {
    ...
    // Verify embedding module exists
    assert!(
...
fn test_a4_device_safe_nan_detection() {
    ...
    match result {
        Ok(_) => {
            ...
            let test_tensor = Tensor::<TestBackend, 2>::from_floats(&[1.0, f32::NAN, 3.0, 4.0], &device)
                .reshape([2, 2]);
            
            // Test device-side NaN detection (should not use to_data())
            let nan_mask = test_tensor.is_nan();
            let has_nan = nan_mask.any().to_element::<f32>() > 0.0;
...
fn test_all_defects_fixed_integration() {
    ...
    // Test RNG isolation
    let rng_context = TorchRngContext::<TestBackend>::new(42, device);
    let _tensor1 = rng_context.create_random_tensor([2, 2], 123);
    let _tensor2 = rng_context.create_random_tensor([2, 2], 123);
    
    // Test device-safe operations
    let test_tensor = Tensor::<TestBackend, 1>::from_floats(&[1.0, 2.0, f32::NAN], &device);
    let has_nan = test_tensor.is_nan().any().to_element::<f32>() > 0.0;
    assert!(has_nan);
