...
use tab_pfn_rs::tabpfn::architectures::base::transformer::{
    DeterministicRngContext, PerFeatureTransformer, DeterministicLinear, DeterministicEmbedding
};
...
fn test_parameter_registration_blocking() {
    ...
    let model = PerFeatureTransformer::<TestBackend>::new(&config, &rng_context);
...
fn test_deterministic_initialization_blocking() {
    ...
    let rng_context1 = DeterministicRngContext::<TestBackend>::new(seed, device);
    let rng_context2 = DeterministicRngContext::<TestBackend>::new(seed, device);
    
    let model1 = PerFeatureTransformer::<TestBackend>::new(&config, &rng_context1);
    let model2 = PerFeatureTransformer::<TestBackend>::new(&config, &rng_context2);
...
fn test_deterministic_forward_blocking() {
    ...
    let mut rng1 = StdRng::seed_from_u64(seed + 1000); // offset for forward randomness
    let mut rng2 = StdRng::seed_from_u64(seed + 1000); // same offset ensures identical forward RNG
...
fn test_deterministic_modules_basic() {
    ...
    let linear = rng_context.create_deterministic_linear::<TestBackend>(3, 4, true, 100);
    let input = Tensor::<TestBackend, 2>::zeros([2, 3], &device);
    let output = linear.forward(input);
    assert_eq!(output.dims(), [2, 4]);
    
    // Test DeterministicEmbedding
    let embedding = rng_context.create_deterministic_embedding::<TestBackend>(10, 5, 200);
