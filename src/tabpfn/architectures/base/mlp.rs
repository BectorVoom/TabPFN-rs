...
mod tests {
    ...
    fn test_weight_layout_conversion() {
        ...
        let (mut mlp, _config) = MLP::<TestBackend>::new(2, 3, Activation::RELU, &device, false, false);
        
        // Set known weights and verify they are stored correctly
        let weight_data = [1.0f32, 2.0f32, 3.0f32, 4.0f32, 5.0f32, 6.0f32];
        let weight_tensor: Tensor<TestBackend, 2> = Tensor::<TestBackend, 1>::from_floats(weight_data.as_slice(), &device)
    ...
    fn test_different_activation_functions() {
        ...
        let (mlp_gelu, config_gelu) = MLP::<TestBackend>::new(3, 4, Activation::GELU, &device, false, false);
        let (mlp_relu, config_relu) = MLP::<TestBackend>::new(3, 4, Activation::RELU, &device, false, false);
        
        let input: Tensor<TestBackend, 1> = Tensor::<TestBackend, 1>::from_floats([1.0f32, -0.5f32, 0.5f32].as_slice(), &device);
    ...
    fn test_batch_processing() {
        let device = Default::default();
        let (mlp, config) = MLP::<TestBackend>::new(2, 3, Activation::GELU, &device, false, false);
        
        // Test with batch size > 1
        let batch_input: Tensor<TestBackend, 2> = Tensor::<TestBackend, 1>::from_floats(
            [1.0f32, 2.0f32, 3.0f32, 4.0f32].as_slice(), &device)
    ...
    fn test_add_input_functionality() {
        let device = Default::default();
        let (mlp, config) = MLP::<TestBackend>::new(3, 4, Activation::GELU, &device, false, false);
        
        let input: Tensor<TestBackend, 1> = Tensor::<TestBackend, 1>::from_floats([1.0f32, 2.0f32, 3.0f32].as_slice(), &device);
    ...
    fn test_memory_optimization() {
        let device = Default::default();
        let (mlp, config) = MLP::<TestBackend>::new(4, 8, Activation::GELU, &device, false, false);
        
        let input: Tensor<TestBackend, 2> = Tensor::<TestBackend, 1>::from_floats(
            [1.0f32, 2.0f32, 3.0f32, 4.0f32, 5.0f32, 6.0f32, 7.0f32, 8.0f32].as_slice(), &device)
            .reshape([2, 4]);
