...
impl<B: Backend> DeterministicRngContext<B> {
    ...
    pub fn create_deterministic_linear<TARGET: Backend>(
        &self,
        input_dim: usize,
        output_dim: usize,
        bias: bool,
        seed: u64,
    ) -> DeterministicLinear<TARGET>
    where
        Tensor<B, 2>: Into<Tensor<TARGET, 2>>,
        Tensor<B, 1>: Into<Tensor<TARGET, 1>>,
    {
        let mut rng = StdRng::seed_from_u64(seed);
        
        // Generate deterministic weights using Xavier/Glorot initialization
        let std = (2.0 / (input_dim + output_dim) as f32).sqrt();
        let weight_inner = self.generate_normal_tensor([output_dim, input_dim], &mut rng, 0.0, std);
        let weight: Tensor<TARGET, 2> = weight_inner.into();
        
        // Generate deterministic bias if needed
        let bias_tensor = if bias {
            let bias_inner = self.generate_normal_tensor([output_dim], &mut rng, 0.0, 0.01);
            let bias: Tensor<TARGET, 1> = bias_inner.into();
            Some(bias)
        } else {
            None
        };
    ...
    pub fn create_deterministic_embedding<TARGET: Backend>(
        &self,
        vocab_size: usize,
        embedding_dim: usize,
        seed: u64,
    ) -> DeterministicEmbedding<TARGET>
    where
        Tensor<B, 2>: Into<Tensor<TARGET, 2>>,
    {
        let mut rng = StdRng::seed_from_u64(seed);
        ...
        let weight_inner = self.generate_normal_tensor([vocab_size, embedding_dim], &mut rng, 0.0, std);
        let weight: Tensor<TARGET, 2> = weight_inner.into();
    ...
    pub fn create_deterministic_layer_norm<TARGET: Backend>(
        &self,
        d_model: usize,
        eps: f64,
    ) -> burn::nn::LayerNorm<TARGET>
    where
        B::Device: Into<TARGET::Device>,
    {
        use burn::nn::LayerNormConfig;
        let config = LayerNormConfig::new(d_model).with_epsilon(eps);
        let target_device: TARGET::Device = self.device.clone().into();
        config.init(&target_device)
    }
    ...
    pub fn create_deterministic_dropout<TARGET: Backend>(
        &self,
        prob: f64,
    ) -> burn::nn::Dropout
    where
    ...
    /// Create a deterministic CrossEntropyLoss
    /// 
    /// CrossEntropyLoss doesn't require random initialization.
    pub fn create_deterministic_cross_entropy_loss<TARGET: Backend>(
        &self,
    ) -> burn::nn::loss::CrossEntropyLoss<TARGET>
    where
        B::Device: Into<TARGET::Device>,
    {
        use burn::nn::loss::CrossEntropyLossConfig;
        let target_device: TARGET::Device = self.device.clone().into();
        CrossEntropyLossConfig::new().init(&target_device)
    }
    ...
    B::InnerBackend: Backend,
{
    ...
    pub fn new(
        ...
        let rng_context = DeterministicRngContext::<B::InnerBackend>::new(config.seed as u64, device.clone());
        ...
        let (global_att_embeddings_for_compression, encoder_compression_layer) =
            if use_encoder_compression_layer {
                ...
                let embeddings = rng_context.create_deterministic_embedding::<B>(
        ...
        let decoder_linear1 = rng_context.create_deterministic_linear::<B>(
            ninp,
            nhid,
            true, // with bias
        ...
        let decoder_linear2 = rng_context.create_deterministic_linear::<B>(
            nhid,
            n_out,
            true, // with bias
        ...
        let feature_positional_embedding_embeddings = match &config.feature_positional_embedding {
            Some(FeaturePositionalEmbedding::Learned) => {
                ...
                Some(rng_context.create_deterministic_embedding::<B>(
    ...
    pub fn transformer_forward(
        ...
        // Add embeddings (feature positional and DAG) with proper slice assignment using isolated RNG
        let (embedded_x, embedded_y) = self.rng_context.with_isolated_seed(Some(*self.seed), |rng| {
            self.add_embeddings(
                embedded_x,
                embedded_y,
                data_dags,
        ...
        let encoder_out = self.rng_context.with_isolated_seed(Some(*self.seed + 1), |layer_rng| {
            self.transformer_encoder.layerstack_forward(
                encoder_input,
        ...
        let final_encoder_out = if let Some(ref mut decoder) = self.transformer_decoder {
            ...
            let test_encoder_out = self.rng_context.with_isolated_seed(Some(*self.seed + 2), |decoder_rng| {
                decoder.layerstack_forward(
                    test_input,
                    Some(0),
                    false,
                    Some(decoder_rng),
...
mod tests {
    ...
    fn test_reproducibility_deterministic() {
        ...
        let mut model3 = PerFeatureTransformer::<TestBackend>::new(
            &cfg_different, 2, "gelu", None, false, None, false, None, false, &device
        ).expect("create model3");

        let (x3, y3) = make_test_input(2, 3, 4, &device);
        let out3 = model3.transformer_forward(x3, Some(y3), true, None, None, None)
            .expect("forward3");
