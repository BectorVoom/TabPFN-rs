...
fn default_attention_init_gain() -> f32 { 1.0 }
...
impl ModelConfig {
    ...
    pub fn upgrade_config(mut config: HashMap<String, serde_json::Value>) -> Result<HashMap<String, serde_json::Value>, String> {
        ...
        if let Some(gain) = config.get("attention_init_gain") {
            if gain.is_null() {
                config.insert("attention_init_gain".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(1.0f32 as f64).unwrap()));
            }
        }
