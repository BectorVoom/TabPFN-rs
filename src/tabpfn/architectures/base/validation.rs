...
use super::train::{DatasetPrior, SyntheticTabularDataset};
...
impl<B: Backend> ValidationEngine<B> {
    ...
    pub fn run_validation(&mut self) -> Result<ValidationResults, Box<dyn std::error::Error>> {
        ...
        for protocol in &self.config.protocols {
            let results = match protocol {
                ValidationProtocol::FewShot(k) => self.run_few_shot_validation(*k)?,
                ValidationProtocol::FullDataset => self.run_full_dataset_validation()?,
                ValidationProtocol::CrossDomain => self.run_cross_domain_validation()?,
                ValidationProtocol::Transductive => self.run_transductive_validation()?,
                ValidationProtocol::Inductive => self.run_inductive_validation()?,
            };
    ...
    fn run_few_shot_validation(&mut self, k: usize) -> Result<Vec<TaskResult>, Box<dyn std::error::Error>> {
        let mut results = Vec::new();

        for task_idx in 0..self.config.num_synthetic_tasks {
            // Sample a synthetic task
            let num_samples = rand::random::<usize>() %
            ...
            // Create few-shot train/test split
            let train_indices = self.sample_indices(k, num_samples);
    ...
    fn run_full_dataset_validation(&mut self) -> Result<Vec<TaskResult>, Box<dyn std::error::Error>> {
        let mut results = Vec::new();
    ...
    fn run_cross_domain_validation(&mut self) -> Result<Vec<TaskResult>, Box<dyn std::error::Error>> {
    ...
    fn run_transductive_validation(&mut self) -> Result<Vec<TaskResult>, Box<dyn std::error::Error>> {
        let mut results = Vec::new();
    ...
    fn run_inductive_validation(&mut self) -> Result<Vec<TaskResult>, Box<dyn std::error::Error>> {
        let mut results = Vec::new();
    ...
    fn compute_metrics(
        ...
        let pred_classes = predictions.argmax(2).squeeze_dim(2);
    ...
    fn sample_synthetic_dataset(&self, num_samples: usize) -> SyntheticTabularDataset {
        ...
        prior.sample::<B>(num_samples, &self.device)
    }
    ...
    fn sample_indices(&self, k: usize, total: usize) -> Vec<usize> {
