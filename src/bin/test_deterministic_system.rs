...
use tabpfn_rs::tabpfn::architectures::base::transformer::{
...
fn main() {
    ...
    let out_values1: Vec<f32> = out_data1.iter().cloned().collect();
    let out_values2: Vec<f32> = out_data2.iter().cloned().collect();
    ...
    let values_s1: Vec<f32> = data_s1.iter().cloned().collect();
    let values_s2: Vec<f32> = data_s2.iter().cloned().collect();
