...
fn test_canonical_mlp(data: CanonicalMLPData) -> TestResult {
    ...
    if let Err(e) = set_mlp_weights_canonical(&mut mlp, &data.weights, &device) {
        return TestResult {
            test_type: "mlp_canonical".to_string(),
            success: false,
            rust_output: None,
    ...
    let input_tensor: Tensor<Backend, 1> = 
        Tensor::<Backend, 1>::from_floats(data.input.as_slice(), &device);
...
fn test_mlp_forward(data: MLPTestData, test_type: String) -> TestResult {
    ...
    if let Err(e) = set_mlp_weights_legacy(&mut mlp, &data.linear1_weight, &data.linear2_weight, &device) {
        return TestResult {
